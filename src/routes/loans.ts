...
loansRouter.post('/', async (req, res) => {
  ...
  if (!book_id || !borrower) return res.status(400).json({ error: 'book_id and borrower required' })
  try {
    const { rows } = await pool.query(
      'INSERT INTO loans(book_id, borrower) VALUES($1,$2) RETURNING *',
      [book_id, borrower]
    )
...
loansRouter.post('/:id/return', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'UPDATE loans SET return_date = CURRENT_TIMESTAMP WHERE id=$1 AND return_date IS NULL RETURNING *',
      [id]
    )
    if (rows.length === 0) return res.status(404).json({ error: 'not found or already returned' })
    res.json(rows[0])
  } catch (e) {
