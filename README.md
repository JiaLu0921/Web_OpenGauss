本项目实现：
- 后端使用 Node.js + Express，连接国产数据库 openGauss（兼容 PostgreSQL 协议，KingbaseES 同样适配）。
- 提供任务管理 CRUD API 与简易前端页面。
- 数据库设计、部署、访问为重点展示：包含模式定义、初始化与连接配置。
...
1. 安装依赖：
   ...
   npm install
   ```
2. 配置环境：复制 `.env.example` 为 `.env` 并按需修改数据库连接。
3. **启动 openGauss 数据库**（Docker，需要 Docker Desktop）：
   ...
   ```
   或者直接用 Docker Compose（包含 Web + DB）：
   ```
...
- openGauss: `localhost:5432`，用户 `gaussdb`，密码来自 `docker-compose.yml` 的 `GS_PASSWORD`。
